...
import { arrowBindings, applyEdits, createDocSession, readObjectById } from '../../../apps/web/src/doc/mutations'
...
describe('planCreate', () => {
  it('lays out unplaced nodes without overlaps, and keeps placed ones where they were put', async () => {
    ...
    const batch = await planCreate(
      session,
      [{ ref: 'a', label: 'A' }, { ref: 'b', label: 'B' }, { ref: 'c', label: 'C' }, { ref: 'pinned', x: -500, y: -500 }],
    ...
    expect(arrowBindings(session, ids.edge1)).toMatchObject({ start: { targetId: ids.a }, end: { targetId: ids.b } })
  })

  it('places a new block beside what is already on the board', async () => {
    const session = fresh()
    applyEdits(session, { create: [{ ref: 'old', object: { type: 'rect', x: 0, y: 100, w: 400, h: 300 } }] })
  ...
  it('writes colours, heads and routing', async () => {
    const session = fresh()
    const a = applyEdits(session, { create: [{ ref: 'a', object: { type: 'rect', x: 0, y: 0 } }] }).ids.a
    const b = applyEdits(session, { create: [{ ref: 'b', object: { type: 'rect', x: 400, y: 0 } }] }).ids.b
    const batch = await planCreate(session, [], [{ from: a, to: b, direction: 'both', routing: 'orthogonal', stroke: '#abc' }])
    const { ids } = applyEdits(session, batch)
  ...
  it('refuses bad colours, self-loops and too much at once', async () => {
    ...
    await expect(planCreate(session, [{ ref: 'a' }], [{ from: 'a', to: 'a' }])).rejects.toThrow(/itself/)
    await expect(planCreate(session, Array.from({ length: 501 }, () => ({})), [])).rejects.toThrow(/at most/)
  })
})
...
describe('return edges', () => {
  it('bow away from an edge already joining the same pair the other way', async () => {
    ...
    const first = applyEdits(
      session,
      await planCreate(session, [{ ref: 'a' }, { ref: 'b' }, { ref: 'c' }], [{ from: 'a', to: 'b' }]),
    )
    const { ids } = applyEdits(
      session,
      await planCreate(session, [], [{ from: first.ids.b, to: first.ids.a }, { from: first.ids.b, to: first.ids.c }]),
    )
    expect(readObjectById(session, ids.edge1)!.props).toMatchObject({ routing: 'curved', curvature: 0.45 })
...
describe('planDiagram', () => {
  it('matches existing nodes by label and does not draw an edge twice', async () => {
    const session = fresh()
    const first = await planDiagram(session, parseMermaid('flowchart LR\n cart[Cart] -->|pay| paid{Paid?}'))
  ...
  it('refuses an ambiguous label by creating rather than guessing', async () => {
    const session = fresh()
    const twice = await planDiagram(session, { nodes: [{ key: 'a', label: 'Same' }, { key: 'b', label: 'Same' }], edges: [] })
